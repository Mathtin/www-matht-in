...
use crate::paths;
use crate::core_dist::{cargo, copy_file_tree_filtered, for_each_file_recursively, make_directory_branch, shell, shell_log_piped, TaskResult, BUILD_PATH, PROJECT_ROOT};
...
fn minify(input: &Path, output: &Path) -> TaskResult {
    ...
    if let Some(dest_path_parent) = output.parent() {
        make_directory_branch(dest_path_parent)?;
    }
...
fn build_web_distribution_by_path(web_dist_path: &Path, wasm_pkg_path: &Path, do_minify: bool) -> TaskResult {
    ...
    make_directory_branch(&web_dist_path)?;
