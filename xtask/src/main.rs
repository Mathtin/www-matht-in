...
use core_dist::{make_each_directory, TaskResult};
use web_dist::{
...
use std::{
    collections::HashMap, env, process
};
...
fn try_main() -> TaskResult {
    ...
    let tasks_map: HashMap<_, _> = TASKS_LIST.iter()
        .map(|(cmd, f, _)| (*cmd, f))
        .collect();
    let fallback_task = &(print_help as TaskFn);

    let chosen_task = match env::args().nth(1) {
        Some(task_name) => tasks_map.get(task_name.as_str()).unwrap_or(&fallback_task),
        None => fallback_task,
    };
...
fn print_help() -> TaskResult {
    ...
    let max_task_name_length = TASKS_LIST.iter()
        ...
        .max()
        .unwrap_or_default();
    let task_width = max_task_name_length - (max_task_name_length % TAB_SIZE) + 2 * TAB_SIZE;
